...
| `@deepseek-ai/dsh-tool-recall` | `recall_memory` | `ctx.tools`, `owning Agent session`, `ctx.sessionProjections (optional, for the memoryIndex read)` | `tool/call`, `tool/result` | - | recall_memory reconstructs compaction-archived spans from the durable log at each memory/archived entry's shadowed seqs, matched by tag. Without the session-projection seam it returns no memories, so its result is deterministic under replay. |
...
Source: [`packages/memory/tool-recall/src/index.ts`](../packages/memory/tool-recall/src/index.ts)
