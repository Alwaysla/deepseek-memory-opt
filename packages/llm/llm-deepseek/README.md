...
- id: llm-deepseek
  ...
  config:
    ...
    degenerateRepeatLimit: 512  # optional positive integer; consecutive identical fragments before the stream is cut as DEGENERATE_REPETITION
...
`degenerateRepeatLimit` bounds a decode-layer collapse: when generation degenerates it streams the same fragment — a padding token (`[PAD]`), one word — as consecutive deltas until the output cap, which under a large `maxTokens` produces a single reply of tens of thousands of repeats and can poison later turns. The stream translator counts consecutive identical non-empty text/reasoning deltas (one shared counter; a different fragment resets it) and, past this limit, cuts the stream with a `finish {kind: 'error'}` carrying code `DEGENERATE_REPETITION` instead of letting the flood run to the budget. The default 512 sits well above any legitimate run of one repeated fragment and far below a real runaway. Like `EMPTY_RESPONSE`, this code is in the default retryable set, so `dsh-llm-retry` re-samples the step; re-sampling normally recovers.
