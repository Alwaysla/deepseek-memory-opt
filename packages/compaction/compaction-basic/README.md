...
Every setting is optional. Top-level policy fields are defaults for every routed model; `modelPolicies` applies partial overrides to exact provider/model pairs. At pressure time, compaction-basic asks the owning LLM adapter for that route's context capacity, subtracts the final header, non-history runtime context, output headroom, and configured safety reserve, then scales ratio budgets over the remaining history capacity. Provider usage totals do not determine this fixed overhead. Unrecognized keys, duplicate targets, mutually exclusive retention forms, and a merged `retainRatio` that is not below `thresholdRatio` fail plugin load. An absolute `retainTokens` budget that is not below its scaled threshold fails on the first resolvable target because that comparison requires model capacity.

| Key | Required | Meaning |
|---|---|---|
| `thresholdRatio` | no (default `0.8`) | Compact at `floor(historyCapacity × ratio)`. |
| `retainRatio` | no (default `0.16`) | Recent history kept verbatim as a fraction of history capacity; mutually exclusive with `retainTokens`. |
| `retainTokens` | no | Absolute recent-history budget kept verbatim; mutually exclusive with `retainRatio` and must be below the resolved threshold. |
| `safetyReserveTokens` | no (default `0`) | Additional fixed reserve subtracted from history capacity after request overhead and output headroom. |
