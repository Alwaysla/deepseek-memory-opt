...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  'packages/memory/memory-core': { kind: 'none', reason: 'Declares the log-only memory/archived event and the memoryIndex projection (UI/replay state); the recall tool and the archival compaction backend own every model-facing effect.' },
  ...
  'packages/client/ui-session-directives': { kind: 'indirect', reason: 'Browser-side projection editor; the Host session-directives capability owns the model-visible runtime context.' },
